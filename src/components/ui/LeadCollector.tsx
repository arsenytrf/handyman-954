...
const STORAGE_KEY = "h954-lead-collector";
...
export default function LeadCollector() {
  ...
  const [formStep, setFormStep] = useState<"description" | "contact" | null>(
    null
  );
      ...
      addBotMsg(
        {
          ...
          options: [
            ...
            { label: "TV / Shelf Mounting", value: "Mounting" },
            { label: "Flooring", value: "Flooring" },
            { label: "Furniture Assembly", value: "Furniture Assembly" },
            { label: "Drywall / Plastering", value: "Drywall" },
  ...
  const handleContactSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      addBotMsg({
        text: `We got it! Someone will reach out shortly. If it\u2019s urgent, call us at ${company.phone}.`,
      });
      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            className="fixed bottom-22 right-6 lg:bottom-6 lg:right-6 z-50 flex items-center gap-2.5 bg-white text-lime-600 pl-4 pr-5 py-3 rounded-full shadow-lg border border-charcoal-200 hover:shadow-xl hover:border-lime-300 transition-all duration-300 cursor-pointer group"
          ...
          >
            <MessageCircle className="h-5 w-5 text-lime-500 group-hover:text-lime-600 transition-colors duration-300" />
            <span className="text-sm font-semibold text-charcoal-700 group-hover:text-charcoal-900 transition-colors duration-300 whitespace-nowrap">
              Need a handyman?
            </span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{
              boxShadow:
                "0 12px 48px rgba(0,0,0,0.14), 0 2px 12px rgba(132,204,22,0.08)",
            }}
          ...
          >
            {/* Header */}
            <div className="flex shrink-0 items-center justify-between px-5 py-3.5 bg-lime-500">
              <div className="flex items-center gap-3">
                <div className="relative flex h-9 w-9 items-center justify-center rounded-full bg-white/20 overflow-hidden">
                  ...
                  <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-green-300 border-2 border-lime-500" />
                </div>
                <div>
            ...
            <div
              ref={scrollRef}
              className="flex-1 overflow-y-auto px-4 py-4 bg-charcoal-50"
            >
              <div className="flex flex-col gap-3">
                <AnimatePresence initial={false}>
                  {messages.map((msg) => (
                    ...
                    >
                      {msg.role === "user" ? (
                        <div className="max-w-[80%] rounded-2xl rounded-br-md border border-charcoal-200 bg-white px-4 py-2.5 text-sm text-charcoal-800 shadow-sm">
                          {msg.text}
                        </div>
                      ) : (
                        <div className="flex max-w-[90%] flex-col gap-2.5">
                          <div className="rounded-2xl rounded-bl-md bg-lime-500 px-4 py-2.5 text-sm leading-relaxed text-white shadow-sm">
                            {msg.text}
                          </div>
                            ...
                            <div className="flex flex-wrap gap-2 pt-1">
                              {msg.options.map((opt) => (
                                <button
                                  ...
                                  className="rounded-full border border-charcoal-200 bg-white px-4 py-2 text-[13px] font-medium text-charcoal-700 transition-all duration-200 hover:border-lime-400 hover:bg-lime-50 hover:text-lime-700 active:scale-[0.97] cursor-pointer shadow-sm"
                                >
                                  {opt.label}
                                </button>
                              ))}
                ...
                <AnimatePresence>
                  {typing && (
                    ...
                    >
                      <div className="flex items-center gap-1.5 rounded-2xl rounded-bl-md bg-lime-500 px-4 py-3 shadow-sm">
                        <span
                          className="lead-typing-dot"
            ...
            {formStep === "description" && (
              <motion.form
                ...
                className="flex shrink-0 items-center gap-2 border-t border-charcoal-200 bg-white px-4 py-3"
              >
                <div className="flex flex-1 items-center gap-2 rounded-full border border-charcoal-200 bg-charcoal-50 px-4 py-2 transition-colors focus-within:border-lime-400">
                  <input
                    type="text"
                    value={descVal}
                    onChange={(e) => setDescVal(e.target.value)}
                    placeholder="Describe the job..."
                    className="flex-1 bg-transparent text-sm text-charcoal-800 outline-none placeholder:text-charcoal-400"
                    autoFocus
                  />
                  <button
                    type="submit"
                    className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-lime-500 text-white transition-all hover:bg-lime-600 active:scale-95 cursor-pointer"
            ...
            {formStep === "contact" && (
              <motion.form
                ...
                className="shrink-0 border-t border-charcoal-200 bg-white px-4 py-3 space-y-2.5"
              >
                <input
                  type="text"
                  value={nameVal}
                  ...
                  required
                  className="w-full rounded-lg border border-charcoal-200 bg-charcoal-50 px-3.5 py-2.5 text-sm text-charcoal-800 outline-none placeholder:text-charcoal-400 transition-colors focus:border-lime-400"
                  autoFocus
                />
                <input
                  type="tel"
                ...
                <button
                  type="submit"
                  className="w-full rounded-lg bg-lime-500 py-2.5 text-sm font-bold text-white transition-all hover:bg-lime-600 active:scale-[0.98] cursor-pointer shadow-sm"
                >
                  SEND REQUEST
                </button>
              </motion.form>
            ...
            {sessionRef.current.step === 6 && !formStep && (
              <div className="shrink-0 border-t border-charcoal-200 bg-white px-4 py-3 flex gap-2">
                <button
                  onClick={handleClose}
                  className="flex-1 rounded-lg border border-charcoal-200 py-2.5 text-sm font-medium text-charcoal-600 transition-all hover:bg-charcoal-50 cursor-pointer"
                >
                  Close
                </button>
                <button
                  onClick={handleReset}
                  className="flex-1 rounded-lg border border-lime-200 py-2.5 text-sm font-medium text-lime-600 transition-all hover:bg-lime-50 cursor-pointer flex items-center justify-center gap-1.5"
                >
                  <RotateCcw size={13} />
                  Start Over
                </button>
